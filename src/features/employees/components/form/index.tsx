...
const ToolsForm: React.FC<{ posNumb: number; toolOptions: Option[] }> = ({
  ...
  const handleAddTool = () => {
    append({
      toolLanguageResourceId: toolOptions[0].value as number,
...
const PositionsForm = () => {
  ...
  const firstPosResOptionsValue = (posResOptions[0]?.value as number) ?? 0;
  const firstToolLanguageOptionsValue =
    (posResMap.get(firstPosResOptionsValue)?.[0]?.value as number) ?? 0;

  const handleAddPosition = () => {
    append({
      positionResourceId: firstPosResOptionsValue,
      toolLanguages: [
        {
          toolLanguageResourceId: firstToolLanguageOptionsValue,
          from: firstYearOptionsValue,
          to: lastYearOptionsValue,
          images: [],
        },
  ...
  return (
    <>
      ...
      <Button className="self-end" size="sm" onClick={handleAddPosition}>
